...
public class Crawler {
	...
	public Crawler(String url, int max, String[] flags) {
		startUrl = url;
		maxUrls = max;
		fac = new Factory();
		if (flags[0].equals("-l")) {
	...
	public ConcurrentHashMap<String, Integer> getCrawledList(){
		return crawledList;
	}
	
	public HashMap<String, Integer> getLexion() {
		return fac.getLexion();
	}

	public void backingUp() {
		System.out.println("backingUp is called");
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				fac.saveTables(crawledList, toCrawlList, maxUrls);
			}
		}, 1000 * 10, 1000 * 60 * 4);
	}
	...
	public void actionSearch() {
		...
		if (mode.equals("mode1")) {
			fac.loadTables(crawledList, toCrawlList, maxUrls);
			search(maxUrls);
		} else {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					while (true) {
						...
						toCrawlList = new ConcurrentHashMap<String,Integer>(crawledList);
						crawledList.clear();
					}
				}
			});
		...
		while (crawledList.size() < maxUrls) {
			...
			do {
				...
				synchronized (this) {
					...
					/*
					 * if (toCrawlList.keySet().iterator().hasNext()) { url =
					 * (String) toCrawlList.keySet().iterator().next(); //
					 * toCrawlList.keySet().iterator().remove(); // Remove URL
					 * from the To Crawl list. toCrawlList.remove(url); } else {
					 * System.out.println("no more links"); break; }
					 */
				}
			...
			if (pageBody != null && pageBody.length() > 0) {
				...
				for (Object link : links) {
					if (!crawledList.containsKey((String) link)) {
						toCrawlList.put((String) link,
								toCrawlList.getOrDefault((String) link, 1) + crawledList.getOrDefault(url,0));
					} else {
						crawledList.put((String) link, crawledList.get(link.toString()) + crawledList.get(url));
					}
				}
